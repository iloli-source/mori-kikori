...
再取得時にサーバーが一時的に 0 件・部分データを返しても、既存の実データを空マークや
より小さい内容で上書きすることはない（内容は追記方向にのみ更新される）。一時的な接続断は
15 秒後に 1 回自動リトライする。
...
さらに直近 7 日は取得済みでも再取得して上書きする（mori の文字起こしは最大 7 日遅れる
ことがあるため、再取得窓を文書化された最大遅延に合わせている）。
